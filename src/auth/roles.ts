...
export const extractRoles = (profile: unknown): string[] => {
  ...
  if (Array.isArray(clientRoles)) {
    const filteredClientRoles = clientRoles.filter((r): r is string => typeof r === 'string');
  ...
  if (Array.isArray(account)) {
    const accountRoles = account.filter((r): r is string => typeof r === 'string');
    return accountRoles;
  }
...
export const hasAdminRole = (profile: unknown): boolean => hasAdministratorRole(profile);

// Alias for support check
export const hasModeratorRole = (profile: unknown): boolean => hasSupportRole(profile);
...
export const hasAnyRole = (profile: unknown, requiredRoles: string[]): boolean => {
  const roles = extractRoles(profile);

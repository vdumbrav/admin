...
export function apiToForm(apiData: Partial<Quest>): QuestFormValues {
  return {
    ...
    // Iterator mapping for 7-day challenge (API → Form simplification)
    // API has 3 fields: iterator, iterator_reward, iterator_resource
    // Form only needs simple structure: { days, reward_map }
...
export function formToApi(formData: QuestFormValues): Partial<Quest> {
  return {
    ...
    // Iterator support for 7-day challenges (Form → API expansion)
    iterator: formData.iterator
      ? {
          days: formData.iterator.days ?? 7,
          reward_map: formData.iterator.reward_map,
          reward_max:
            formData.iterator.reward_map.length > 0 ? Math.max(...formData.iterator.reward_map) : 0,
          reward: formData.iterator.reward_map[0] ?? 0,
          day: 0,
        }
      : undefined,

    // Duplicate rewards as strings for mobile client compatibility
    iterator_reward: formData.iterator?.reward_map.map((r: number) => r.toString()),

    // UI resources are generated by backend, not form-controlled
    iterator_resource: undefined,
  };
}

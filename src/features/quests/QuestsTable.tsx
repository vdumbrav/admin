    ...
    <div className='space-y-4'>
      <div className='flex flex-wrap gap-2'>
        ...
        <select
          ...
          value={group}
          onChange={(e) =>
            changeFilter(setGroup, e.target.value as 'all' | Task['group'])
          }
        >
        ...
        <select
          className='input'
          value={visible}
          onChange={(e) => changeFilter(setVisible, e.target.value)}
        >
      ...
      {isAdmin && selected.length > 0 && (
        <div className='flex gap-2'>
          <Button
            size='sm'
            onClick={() => {
              bulk.mutate({ ids: selected, action: 'hide' })
              setSelected([])
            }}
          >
            Hide
          ...
          <Button
            size='sm'
            onClick={() => {
              bulk.mutate({ ids: selected, action: 'show' })
              setSelected([])
            }}
          >
            Show
              ...
              <td>
                {isAdmin ? (
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      toggle.mutate({ id: t.id, visible: v })
                    }
                  />
                ) : t.visible ? (
                  'Yes'
                ...
                <td>
                  <div className='flex gap-2'>
                    <Button asChild size='sm' variant='secondary'>
                      <Link to='/quests/$id' params={{ id: String(t.id) }}>
                        Edit
                      </Link>
                    </Button>
                    <Button

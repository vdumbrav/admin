...
let globalAuthenticatedMutator: (<TResponse>(config: AxiosRequestConfig) => Promise<TResponse>) | null = null

// Function to set the authenticated mutator (called by AuthProvider)
export function setAuthenticatedMutator(mutator: <TResponse>(config: AxiosRequestConfig) => Promise<TResponse>) {

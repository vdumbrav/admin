...
- [ ] Создать определения типов `PresetConfig`
- [ ] Реализовать конфигурации пресетов (5 пресетов)
- [ ] Создать компонент `PresetPicker`
- [ ] Добавить роутинг: `/quests/new` → выбор пресета, `/quests/new/:preset` → форма

### 7.2 Фаза 2: Улучшение формы
- [ ] Модифицировать `QuestForm.tsx` для принятия `PresetConfig`
- [ ] Реализовать логику видимости полей
- [ ] Добавить механизм блокировки полей
- [ ] Создать специальные компоненты (TwitterPreview, TasksEditor, DailyRewardsEditor)

### 7.3 Фаза 3: Бизнес-логика
- [ ] Реализовать валидацию Connect-gate
- [ ] Добавить логику расчета наград
- [ ] Реализовать правила валидации, специфичные для пресета
- [ ] Добавить защиту от потери несохраненных изменений

### 7.4 Фаза 4: Интеграция и полировка
- [ ] Интегрировать с существующими API адаптерами
...
- [ ] 5 пресетов реализованы (Connect, Join, Action with post, 7-day challenge, Explore)
- [ ] Выбор пресетов с адаптивным макетом карточек
- [ ] Универсальная форма с динамической конфигурацией полей
- [ ] Валидация и бизнес-правила, специфичные для пресета
- [ ] Логика Connect-gate правильно настроена
- [ ] Превью Twitter для пресета Action with post
- [ ] Редактор тасков с опциями Like/Comment/Retweet
- [ ] Редактор ежедневных наград для 7-day challenge
- [ ] Расчет общей награды для многозадачных квестов
- [ ] Настройка текста кнопок через `resources.ui.button`

### 16.2 Технические требования ✅
- [ ] Архитектура единого компонента формы ("под капотом - одна форма")
- [ ] Система конфигурации пресетов
- [ ] Совместимость с существующим API адаптером
- [ ] Безопасность типов с TypeScript
- [ ] Валидация формы со схемами Zod
- [ ] Защита несохраненных изменений
- [ ] Состояния загрузки и обработка ошибок
- [ ] Адаптивный дизайн (мобильный, планшет, рабочий стол)

### 16.3 UX требования ✅
- [ ] Выбор пресетов с эффектами наведения
- [ ] Автофокус на поле заголовка формы
- [ ] Дата/время начала по умолчанию сейчас + 1 час
- [ ] Обновления общей награды в реальном времени
- [ ] Визуальные индикаторы блокировки полей
- [ ] Сообщения об ошибках под соответствующими полями
- [ ] После успешного сохранения → тост «Quest saved successfully» → редирект на `/quests`
- [ ] Правильный поток навигации (выбор → форма → список)
- [ ] URL для Explore валидируется и не дает сохранить пустым/без схемы
- [ ] При Group = Partner в Join/Action блок загрузки иконки виден; в Explore — всегда
...
*Эта спецификация служит окончательным руководством для реализации Эпика пресетов квестов. Вся работа по разработке должна ссылаться на этот документ для требований, технических решений и критериев приемки.*